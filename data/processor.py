    ...
    def _read_json(cls, input_file):
        ...
        with open(input_file, "r", encoding='utf-8') as reader:
            ...
            for line in all_lines:
                ins = eval(line)  #line:string type  ins:dictionary type
                data.append(ins)
        return data
...
def convert_examples_to_features(examples, max_seq_length, tokenizer, args, rel2id):
    ...
    if False:
        ...
        for (ex_index, example) in enumerate(examples):
            ...
            # SUBJECT_ids1 = tokenizer(SUBJECT, add_special_tokens=False)
            # a = tokenizer.tokenize(SUBJECT)
            # b = tokenizer.tokenize(" "+SUBJECT)
            if use_gpt:
                if args.CT_CL:
            ...
            inputs = tokenizer(
                prompt,
                " ".join(tokens),
                truncation="longest_first", # When the text is too long, it is truncated to the maximum length specified by the max_length parameter
                max_length=max_seq_length,
                padding="max_length",
                add_special_tokens=True  # True is to add special symbols to the sequence, such as [CLS], [SEP]
            )
            decoded_input = tokenizer.decode(inputs['input_ids'])
            #<s>[sub] water [sub]<mask> [obj] oven [obj].</s></s>he poured all the [subject_start] water [subject_end] he got out from the [object_start] oven [object_end] and went tothe boyer.</s>
