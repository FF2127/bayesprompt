...
def f1_eval(logits, labels):
    def getpred(result, T1 = 0.5, T2 = 0.4) :
        # Use the threshold to get preds, result = logits
        # T2 means that if they are all lower than T2, then it is no relation, otherwise choose the largest one
        ret = []
    ...
    def geteval(devp, data):
        ...
        for i in range(len(data)):
            # Each sample is [1,4,...,20] means 1, 4, 20 is 1, if not [36]
            for id in data[i]:
                if id != 36:
                    # the number of 1's in the label
                    correct_gt += 1
                    if id in devp[i]:
                        # correct prediction
                        correct_sys += 1
